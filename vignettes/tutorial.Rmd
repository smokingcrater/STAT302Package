...
In the following hypothesis test, $\alpha = 0.05$.  The resulting p-value is substantially greater than $\alpha$, thus indicating weak evidence against the null hypothesis.  Therefore we fail to reject the null hypothesis.
...
The following scatterplot illustrates the relationship between the actual and fitted values: blah, blah, blah
...
# Successively apply nearest neighbor algorithm using cross-validatio for
...
Both the training misclassification rates (no cross-validation) and the cross-validation misclassification rates for the successive calls (1 to 10 neighbors) are documented in the following table:
...
In theory, though I would choose model 1 (1 neighbor) based on the training misclassification rates since it returns the lowest error.  Likewise, I would choose model 1 (1 neighbor) based on the training missclassification rates since it's 0.

In practice, however, taking into account both the blah, blah, blah
